<?php
	...
	add_shortcode('custom_select', function($atts) {
		...
		ob_start();
		?>
		<div class="custom-select default-select" data-placeholder="<?= esc_html($atts['placeholder']) ?>" data-name="<?= esc_attr($atts['name']) ?>">
			...
			<ul class="options-container">
				<?php foreach ($options as $index => $value): ?>
					...
					$classes = 'option' . ($index === 0 ? ' active' : '') . ($is_disabled ? ' disabled' : '');
					?>
